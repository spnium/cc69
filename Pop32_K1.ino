...
const int threshold[numSensors] = {800, 800, 800, 800, 800, 800, 800, 800};
...
//////////////////////////////////////////////////////////////////////////
#define turn1  168        //หน่วงเวลาในการเลี้ยวซ้าย  ****ใส่เป็นเวลา****** //L
#define turn2  185//275        //หน่วงเวลาในการเลี้ยวขวา  ****ใส่เป็นเวลา****** //R
...
long CH[9] = {0, 200, 400, 600, 950, 1100, 1200, 1400};
//จับเส้นระยะการเคลื่อนทีแต่ละช่อง {0,1ช่อง,2ช่อง,3ช่อง,4ช่อง,5ช่อง,6ช่อง,7ช่อง} //หาค่าก่อนใช้งาน

///////////////////////////////////////////////////////////////////////////
long CH1[9] = {0, 280, 480, 695, 950, 1100, 1200, 1400}; //ไม่จับเส็น ใช่กับ T และ FT
...
void setup() {
  ...
  // calibrate();while (1);
  // printSensor();
  motor(__MTR_ALL, 80); delay(100);AO();
  FT(1);
  FC(2);
  R1();
  FC(2);
...
void RC (int TIME) {
  ...
  while (analogRead(BL) < sensorBL || analog(BR) < sensorBR) {
    ...
    //   motor(12, 0 );  //ล้อซ้ายความเร็วเดินหน้าเช็คช้า
    //   motor(34, -45 );  //ล้อขวาความเร็วเดินหน้าเช็คช้า
    // }
    // else if (analogRead(BL) < sensorBL  && analogRead(BR) > sensorBR) {
    //   motor(12, -45 );  //ล้อซ้ายความเร็วเดินหน้าเช็คช้า
    //   motor(34, 0 );  //ล้อขวาความเร็วเดินหน้าเช็คช้า
    // }
    else if (analogRead(BL) > sensorBL && analog(BR) > sensorBR){
      break;
    }
  }
  AO();
